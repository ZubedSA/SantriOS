...
## 13. Invarian Paritas Navigasi Mobile & Sinkronisasi Kartu Dashboard (Navigation Parity Invariant)
- **100% Feature Reachability on Mobile**: Setiap halaman dan sub-modul yang tersedia di Desktop Sidebar wajib dapat diakses pada Mode Mobile (< 768px). Mobile Bottom Bar menyediakan 4 menu utama peran + 1 tombol "Menu" (Mobile Drawer) yang menampilkan seluruh hierarki menu sidebar desktop secara lengkap, rapi, dan terkelompok. Tombol hamburger di mobile top header juga wajib dapat membuka Mobile Drawer tersebut.
- **Kesesuaian Tautan Tombol & Kartu Dashboard (Single Source of Truth)**: Semua tombol aksi cepat, kartu KPI, dan link pintasan di tampilan dashboard (`owner-view`, `admin-view`, `guru-view`, `musyrif-view`, `kesantrian-view`, `wali-view`, dll.) wajib menautkan langsung ke rute modul resmi yang terdaftar di sidebar (seperti `/dashboard/akademik`, `/dashboard/tahfizh`, `/dashboard/kesantrian`, `/dashboard/ppdb`, `/dashboard/perizinan`, `/dashboard/kelas`, `/dashboard/nilai`, `/dashboard/asrama`, `/dashboard/surat`). Dilarang menggunakan rute agregat usang atau query string usang (`activities?tab=...` atau `santri?tab=...`).
- **Konsistensi Ikon & Label**: Penamaan menu dan pemilihan ikon Lucide pada Bottom Bar, Mobile Drawer, dan Sidebar Desktop harus seragam untuk mencegah kebingungan pengguna antar perangkat.
